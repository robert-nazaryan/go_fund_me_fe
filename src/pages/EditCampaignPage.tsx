...
function EditCampaignPage() {
    ...
    const loadCampaign = async () => {
        try {
            const response = await campaignAPI.getById(Number(id));
            ...
            if (campaign.documentUrl) {
                setExistingDocumentUrl(campaign.documentUrl);
            }
        } catch (error) {
            console.error('Error loading campaign:', error);
            setError(t.editCampaign.loadError);
        }
    };
    ...
    return (
        <div className="create-campaign-page">
            <div className="create-container">
                <h1>{t.editCampaign.title}</h1>
                <p>{t.editCampaign.subtitle}</p>
                ...
                <form onSubmit={handleSubmit} className="campaign-form">
                    ...
                    <div className="form-group">
                        <label className="form-label">
                            {t.createCampaign.coverImageLabel}
                        </label>
                        <p style={{fontSize: '14px', color: '#7f8c8d', marginBottom: '12px'}}>
                            {t.editCampaign.coverImageDesc}
                        </p>
                    ...
                    <div className="form-group">
                        ...
                        {(document || existingDocumentUrl) && !removeDocument && (
                            <div style={{
                                marginTop: '16px',
                                padding: '12px',
                                background: '#f8f9fa',
                    ...
                    <span style={{fontSize: '14px', fontWeight: '600'}}>
                      📄 {document ? document.name : t.editCampaign.existingDocument}
                    </span>
                                <button
                                    type="button"
                    ...
                    <div className="form-actions">
                        <button type="button" className="btn" onClick={() => navigate('/my-campaigns')}>
                            {t.createCampaign.cancel}
                        </button>
                        <button type="submit" className="btn btn-primary" disabled={loading}>
                            {loading ? t.editCampaign.saving : t.editCampaign.save}
                        </button>
                    </div>
                </form>
            </div>
